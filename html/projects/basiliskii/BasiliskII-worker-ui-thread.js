...
audio.bytesPerSample = (audio.format == 0x0008 /*AUDIO_U8*/ || audio.format == 0x8008 /*AUDIO_S8*/) ? 1 : 2;
audio.bufferSize = audioTotalSamples*audio.bytesPerSample;
audio.bufferDurationSecs = audio.bufferSize / audio.bytesPerSample / audio.channels / audio.freq; // Duration of a single queued buffer in seconds.
...
var AUDIO_DATA_BUFFER_SIZE = audioBlockChunkSize*audio.maxBuffersInSharedMemory;
...
function openAudio() {
  audio.pushAudio = function pushAudio(
    ...
    var soundBuffer = audioContext.createBuffer(audio.channels, sizeSamplesPerChannel, audio.freq);
    // source.connect(audioContext.destination);
    source.connect(gainNode);

    audio.fillWebAudioBufferFromChunk(blockBuffer, sizeSamplesPerChannel, soundBuffer);
  ...
  audio.fillWebAudioBufferFromChunk = function fillWebAudioBufferFromChunk(
    ...
    for(var c = 0; c < audio.channels; ++c) {
      ...
      if (channelData.length != blockSize) {
        throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + blockSize + ' samples!';
      }
  ...
  audio.caller = function audioCaller() {
    ...
    if (audio.numAudioTimersPending < audio.numSimultaneouslyQueuedBuffers) {
      ++audio.numAudioTimersPending;
      audio.timer = setTimeout(audio.caller, Math.max(0.0, 1000.0*(secsUntilNextPlayStart-preemptBufferFeedSecs)));
